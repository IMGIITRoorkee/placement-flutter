...
class ResultService {
  ...
  Future<List<CompanyConciseModel>?> companyWiseResults(
      int yearIndex, int internSwitch) async {
    List<CompanyConciseModel> _companyResults = [];
    if (_cache.companyWiseResults == null) {
      var _data = await _requestService.makeGetRequest(EndPoints.RESULTS_HOST +
          EndPoints.year(yearIndex) +
          EndPoints.RESULTS_COMPANY[internSwitch] +
  ...
  Future<List<BranchConciseModel>?> branchWiseResults(
      int yearIndex, int internSwitch) async {
    List<BranchConciseModel> _branchResults = [];
    if (_cache.branchWiseResults == null) {
      var _data = await _requestService.makeGetRequest(EndPoints.RESULTS_HOST +
          EndPoints.year(yearIndex) +
          EndPoints.RESULTS_BRANCH[internSwitch] +
          EndPoints.WITH_INDEX);
      if (_data == -2) return null;
      if (_data != -1 && _data != -2) {
        for (var r in _data) {
        ...
        if (_branchResults.length > 0)
          _cache.branchWiseResults = _branchResults;
      }
  ...
  Future<List<CompantWiseStudentModel>> companyWiseDetailResult(
      String args) async {
    List<CompantWiseStudentModel> _results = [];
    var _data =
        await _requestService.makeGetRequest(EndPoints.RESULTS_HOST + args);
    for (var r in _data) {
  ...
  Future<List<BranchWiseStudentModel>> branchWiseDetailResult(
      String args) async {
